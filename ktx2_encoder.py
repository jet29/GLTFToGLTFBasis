    ...
    for line in read_obj:
        ...
        if foundTexture:
            ...
            if "name" in line:
                ...
                replacedLine = line.replace(splitted[1],fileBase + '.ktx2" \n')
        ...
        if foundImage:
            ...
            if "mimeType" in line:
                splitted = line.split(":")
                replacedLine = line.replace("image/png","image/ktx2")
                write_obj.write(replacedLine)
                writeLine = False
...
def imageToBasis(file,dir_path,subdir):
    dest = os.path.join(subdir, file)
    cmd = 'basisu -ktx2 ' + dest
    print(cmd)
    fileName = os.path.splitext(file)[0] + '.ktx2'
